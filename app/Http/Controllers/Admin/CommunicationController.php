...
class CommunicationController extends Controller
{
    ...
    public function getInboxUpdates(Request $request)
    {
            ...
            $threads = $filteredMessages
                ...
                ->map(function ($message) use ($admin) {
                    ...
                    return [
                        ...
                        'other_user_name' => $otherUser->name ?? 'Exhibitor',
        ...
        return response()->json([
            ...
            'threads' => $threads,
            'unread_count' => $unreadCount,
            'inbox_count' => $messages->where('receiver_id', $admin->id)->where('status', '!=', 'archived')->where('status', '!=', 'deleted')->count(),
            'sent_count' => $messages->where('sender_id', $admin->id)->where('status', '!=', 'archived')->where('status', '!=', 'deleted')->count(),
